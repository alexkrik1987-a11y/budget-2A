...
assert.equal(htmlStyleAsset, "styles.css?v=605", "HTML должен подключать новую версию стилей");
assert(sw.includes(`./${htmlStyleAsset}`), "Service Worker должен кешировать ту же версию CSS");
assert(sw.includes('const CACHE_NAME = "budget-2a-v91-parent-ux-1";'), "cache name должен быть обновлён для редизайна");
