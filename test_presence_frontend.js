...
assert.equal(htmlAppVersion, "89", "index.html должен подключать app.js?v=89");
assert.equal(workerAppVersion, htmlAppVersion, "app.js asset version должна совпадать в HTML и Service Worker");
assert.equal(htmlStyleVersion, "605", "index.html должен подключать styles.css?v=605");
assert.equal(workerStyleVersion, htmlStyleVersion, "styles.css asset version должна совпадать в HTML и Service Worker");
assert(serviceWorker.includes('const CACHE_NAME = "budget-2a-v91-parent-ux-1";'), "Service Worker cache name должен быть обновлён");
