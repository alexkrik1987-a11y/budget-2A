...
assert.equal(htmlStyleVersion, "591", "index.html должен подключать styles.css?v=591");
assert.equal(workerStyleVersion, htmlStyleVersion, "styles.css asset version должна совпадать в HTML и Service Worker");
assert(serviceWorker.includes('const CACHE_NAME = "budget-2a-v86-artistic-chalkboard-5";'), "Service Worker cache name должен быть обновлён");
