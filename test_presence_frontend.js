...
assert.equal(htmlAppVersion, "83", "index.html должен подключать app.js?v=83");
assert.equal(workerAppVersion, htmlAppVersion, "app.js asset version должна совпадать в HTML и Service Worker");
assert.equal(htmlStyleVersion, "592", "index.html должен подключать styles.css?v=592");
assert.equal(workerStyleVersion, htmlStyleVersion, "styles.css asset version должна совпадать в HTML и Service Worker");
assert(serviceWorker.includes('const CACHE_NAME = "budget-2a-v86-artistic-chalkboard-6";'), "Service Worker cache name должен быть обновлён");
